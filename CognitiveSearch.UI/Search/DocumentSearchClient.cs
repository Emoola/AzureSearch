...
namespace CognitiveSearch.UI
{
    public class DocumentSearchClient
    {
        ...
        public DocumentSearchResult GetFacets(string searchText, string facetName, int maxCount = 30)
        {
            ...
            // SearchClientHelper searchClient = new SearchClientHelper(searchServiceName, apiKey, IndexName);
            //  facetName = "terms";
            // Execute search based on query string	
            try
            {
                SearchParameters sp = new SearchParameters()
                {
                    ...
                    Facets = new List<String>() { $"{facetName}, count:{maxCount}" }, // Facets = new List<String>() { "terms, count:" + maxCount },
                    QueryType = QueryType.Full
                };
